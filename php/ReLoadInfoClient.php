...
if(!$conn) {
  exit('Could not connect');
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<th>Date  </th>";
        echo "<th>Heure  </th>";
        echo "<th colspan='3' style='position:absolue;right:0px;'>Actions  </th>";
	    echo "</tr>";
	  while($row = mysqli_fetch_assoc($result)) {
	    ...
	    echo "</th>";
	    echo "<th>";
	    echo "<div  d-flex justify-content-center'> <button type='button' class='btn btn-danger btn-lg' data-toggle='modal' data-target='#exampleModal' onclick='DeleteRdvServer(".json_encode($row).")'>DELETE</button></div>";
	    echo "</th>";
	    echo "</td>";
        echo "</th>";
	  }
